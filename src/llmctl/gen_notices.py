...
def link(key: str, url: str | None = None) -> str:
    ...
    if key.startswith("github.com/"):
        return "[{}](https://{})".format(key[len("github.com/") :], key)
    return "[{}]({})".format(key, url or "https://" + key)
...
def collect(marketplace: str | Path) -> list[dict]:
    plugins = Path(marketplace) / "plugins"
    if not plugins.is_dir():
        return []
    bundles = []
    for child in sorted(plugins.iterdir()):
        entry, path = child.name, str(child)
        if child.is_dir():
            bundles.append(
                {
                    "name": entry,
...
# What a bundle's own section reports: the upstream it vendors whole, and the
# files it adapted. `Writer` is `out.append` -- every section appends lines to
# the one document being built.
Writer = Callable[[str], None]

Notice = tuple[str, str, str, str, str]
Exception_ = tuple[str, str, str, str, str, str, str, str]


def preamble(w: Writer) -> None:
    """What the document is, and what each of its two lists means."""
    w("# Third-party notices")
    w("")
    w("<!-- Generated by llmctl-gen-notices in .llmctl. Do not edit by hand;")
    w("     re-run it (or a release) instead. -->")
    w("")
    w(
        "Plugin bundles here vendor skills authored elsewhere, and adapt content from "
    ...
    w("")
    w(
        "**Notices** lists sources whose terms attach to what we redistribute. "
    ...
    w("")
    w(
        "Each bundle's `apm.lock.yaml` records the upstream `repo_url`, the resolved "
...
def bundle_rows(
    bundle: dict, licenses: dict, ack: dict[str, tuple[set[str], str]]
) -> tuple[list[Notice], list[Exception_]]:
    """One bundle's obligations, split by where they come from.

    Anything the bundle carries without an obligation lands in `ack` instead:
    it is credited once across the whole document, not once per bundle.
    """
    notices: list[Notice] = []
    ...
    for dep in bundle["deps"]:
        ...
        notices.append(
            (
                ...
                info.get("spdx", "?"),
                info.get("holder", ""),
                str(dep.get("resolved_commit", ""))[:12],
            )
        )
...
def bundle_section(
    w: Writer, name: str, notices: list[Notice], exceptions: list[Exception_]
) -> None:
    """One `## <bundle>` heading and the two tables under it."""
    w(f"## {name}")
    w("")
    if notices:
        ...
        w("| --- | --- | --- | --- | --- |")
        for dep_name, key, spdx, holder, commit in sorted(notices):
            w(f"| `{dep_name}` | {link(key)} | {spdx} | {holder or '—'} | `{commit or '—'}` |")
        w("")
    if not exceptions:
        return
    w("### Adapted content")
    ...
    for rel, effective, spdx, fidelity, key, holder, path, url in sorted(exceptions):
        source = f"{link(key)} — [`{path}`]({url})" if path else link(key, url)
        w(f"| `{rel}` | {effective} | {source} | {spdx} ({fidelity}) | {holder or '—'} |")
    w("")
    w(
        "Each file above is a **modification** of its upstream. Where the "
...
def notice_files_section(w: Writer, bundles: list[dict], licenses: dict) -> None:
    ...
    texts = [
        (k, licenses[k]["notice"]) for k in sorted(carried) if licenses.get(k, {}).get("notice")
    ]
    if not texts:
        return
    w("## NOTICE files")
    ...
    for key, text in texts:
        w("**{}**".format(key.replace("github.com/", "")))
        w("")
        w("```text")
...
def acknowledgements_section(
    w: Writer, ack: dict[str, tuple[set[str], str]], licenses: dict
) -> None:
    """Credit for what shaped the work without imposing a condition."""
    if not ack:
        return
    w("## Acknowledgements")
    w("")
    w(
        "Sources that shaped this work without imposing a condition — an idea, a "
    ...
    for key in sorted(ack):
        ...
        info = licenses.get(key, {})
        spdx = info.get("spdx")
        licence = "no licence" if spdx == "NONE" else (spdx or "not recorded")
        # Attribution carries the identity, which for a source keyed by an
        # ISBN or DOI is the only human-readable thing in the row.
        used = ", ".join(sorted(fidelities))
        w(f"| {link(key, url)} | {info.get('holder', '') or '—'} | {licence} | {used} |")
    w("")


def licence_texts_section(w: Writer) -> None:
    """Where the full texts live, and what each licence asks for."""
    w("## Licence texts")
    w("")
    w(
        "Full texts ship in each bundle's `LICENSES/` directory and at the repository "
...
def render(bundles: list[dict], licenses: dict) -> str:
    """The whole notices document, section by section, in reading order."""
    out: list[str] = []
    w = out.append
    preamble(w)

    ack: dict[str, tuple[set[str], str]] = {}  # slug -> (reasons, url), across all bundles
    for bundle in bundles:
        notices, exceptions = bundle_rows(bundle, licenses, ack)
        if notices or exceptions:
            bundle_section(w, bundle["name"], notices, exceptions)

    notice_files_section(w, bundles, licenses)
    acknowledgements_section(w, ack, licenses)
    licence_texts_section(w)
    return "\n".join(out)
...
def render_for(ws: str | Path, marketplace: str | Path) -> str:
    ...
    if not bundles:
        raise RuntimeError(f"no packed bundles under {marketplace}/plugins")
...
def unrecorded(text: str) -> list[str]:
    ...
    return sorted(
        {
            m.group(1)
            for m in re.finditer(rf"\| `[^`]*` \| \[([^\]]+)\]\([^)]*\) \| {UNRECORDED} \|", text)
        }
    )
...
def relicensing(licenses: dict, gh: githublib.GitHub) -> list[Checked]:
    ...
    for key, info in sorted(licenses.items()):
        ...
        try:
            ...
            rows.append(Checked(key, recorded, "", f"could not query ({exc})"))
            continue
        if reported == recorded:
            rows.append(Checked(key, recorded, reported, "ok"))
        elif reported == "NOASSERTION":
            rows.append(Checked(key, recorded, reported, "ok (API cannot classify it; see note)"))
        else:
...
def verify(licenses: dict, gh: githublib.GitHub, log: Log = print) -> int:
    ...
    for row in rows:
        log(f"  {row.key:<44} {row.recorded:<14} {row.status}")
    drift = [row for row in rows if row.status == "drift"]
    if drift:
        log(
            f"\n{len(drift)} upstream(s) may have relicensed. Re-read their LICENSE "
            f"file and update dependency-licenses.yml."
        )
...
def main(
    ...
    if check:
        ...
        if existing.replace("\r\n", "\n") != text:
            sys.stderr.write("THIRD-PARTY-NOTICES.md is out of date - run `apm run gen-notices`\n")
            raise typer.Exit(1)
    ...
    if missing:
        sys.stderr.write(
            "no licence recorded for: {} - add them to dependency-licenses.yml\n".format(
                ", ".join(missing)
            )
        )
    print(f"[notices] {target} ({len(collect(marketplace))} bundle(s))")

...
across a `..` boundary:
  ...
  2. marketplace  emit apm.yml from apm.marketplace.yml plus one entry per
                  package's apm.yml, copy the other `*.marketplace.*`
                  sources, delete everything that is not expected output,
                  collect the bundles' always-on instructions into
                  ALWAYS_ON_INSTRUCTIONS.md, `apm pack`, regenerate the notices
...
from . import check_frontmatter, gen_notices, provenance, workspace
...
# What each generated catalogue entry carries from the package's apm.yml.
# `source` and `version` are filled in by the packer.
ENTRY_KEYS = ("name", "description", "category")
...
def read_catalogue(ws: Path) -> Any:
    ...
    if ((data or {}).get("marketplace") or {}).get("packages"):
        raise PackError(
            f"{CATALOGUE} lists packages; the list is generated from packages/*/apm.yml "
            "now -- move each entry's `category` into its package's apm.yml and drop the list"
        )
    return data


def catalogue_entries(packages: Sequence[workspace.Package]) -> list[dict[str, str]]:
    """One catalogue entry per package, read from its apm.yml.

    `category` is not an APM key; APM ignores it in apm.yml, and Codex refuses a
    marketplace entry without one.
    """
    entries = []
    for package in packages:
        entry = {key: package.manifest.get(key) for key in ENTRY_KEYS}
        missing = [key for key, value in entry.items() if not value]
        if missing:
            keys = ", ".join(f"`{key}`" for key in missing)
            raise PackError(
                f"packages/{package.directory}/apm.yml is missing {keys} -- "
                "the marketplace entry is built from it"
            )
...
def emit_catalogue(
    ws: Path, marketplace: Path, entries: Sequence[dict[str, str]], versions: dict[str, str]
) -> None:
    """Write <marketplace>/apm.yml: apm.marketplace.yml plus one entry per package."""
    data = read_catalogue(ws)
    data["marketplace"]["packages"] = [
        {
            "name": entry["name"],
            "source": "./plugins/{}-{}".format(entry["name"], versions[entry["name"]]),
...
ALWAYS_ON_INSTRUCTIONS = "ALWAYS_ON_INSTRUCTIONS.md"


def always_on_body(text: str) -> str | None:
    """The body of an instruction without `applyTo`, the ones every session loads."""
    frontmatter = check_frontmatter.extract(text)
    if frontmatter is None:
        return text.strip() or None
    if check_frontmatter.field(frontmatter, "applyTo") is not None:
        return None
    body = text.split("\n---", 1)[1].partition("\n")[2]
...
def write_always_on_instructions(marketplace: Path, bundles: Sequence[str]) -> None:
    """Collect each bundle's always-on instructions for surfaces that load no plugin.

    claude.ai, ChatGPT and github.com take personal instructions only as text
    pasted into their settings. Generated from the bundles rather than the
    workspace so the text matches the released version of each plugin.
    """
    sections = []
    for bundle in bundles:
        bundle_dir = marketplace / "plugins" / bundle
        files = sorted((bundle_dir / "instructions").glob("*.instructions.md"))
        bodies = [b for f in files if (b := always_on_body(f.read_text(encoding="utf-8")))]
        if not bodies:
            continue
    ...
    header = (
        ...
        "The always-on instructions each plugin carries, for pasting into the personal "
        "settings of a surface that loads no plugins. Where to paste them is in the "
        "[README](README.md#always-on-instructions). Generated by llmctl-pack-marketplace; "
        "do not edit here.\n"
    )
    text = header + "".join(f"\n{section}\n" for section in sections)
    (marketplace / ALWAYS_ON_INSTRUCTIONS).write_text(text, encoding="utf-8")
...
def pack_all(
    ...
    *,
    ...
    read_catalogue(ws)
    entries = catalogue_entries(packages)
    categories = {entry["name"]: entry["category"] for entry in entries}

    missing = {p.name for p in packages} - set(versions)
    if missing:
    ...
    for package in packages:
        ...
        packer.pack(package, version, categories[package.name])
        packed.append(bundle)

    emit_catalogue(ws, marketplace, entries, versions)
    ...
    write_always_on_instructions(marketplace, [f"{n}-{v}" for n, v in versions.items()])

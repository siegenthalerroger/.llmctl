...
across a `..` boundary:
  ...
  2. marketplace  emit apm.yml from apm.marketplace.yml plus one entry per
                  ...
                  PERSONAL-INSTRUCTIONS.md, `apm pack`, regenerate the notices
...
PERSONAL_INSTRUCTIONS = "PERSONAL-INSTRUCTIONS.md"
...
def write_personal_instructions(marketplace: Path, bundles: Sequence[str]) -> None:
    ...
    header = (
        ...
        "[README](README.md#personal-instructions). Generated by llmctl-pack-marketplace; "
        "do not edit here.\n"
    )
    text = header + "".join(f"\n{section}\n" for section in sections)
    (marketplace / PERSONAL_INSTRUCTIONS).write_text(text, encoding="utf-8")
...
def pack_all(
    ...
    *,
    ...
    write_personal_instructions(marketplace, [f"{n}-{v}" for n, v in versions.items()])

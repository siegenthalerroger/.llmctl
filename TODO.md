...
- [scripts/check.py](scripts/check.py) (frontmatter, licences) and [scripts/release-check.py](scripts/release-check.py) (notices, versions, manifest drift, bundle validity). These are repo-wide and run in CI, but they only cover provenance/licensing and the release path — not the frontmatter conventions below.
fir
...
- [ ] **4g — Enforce the commit convention with a `commit-msg` hook.** The format is documented in [CONTRIBUTING.md](CONTRIBUTING.md#commit-convention) and [AGENTS.md](AGENTS.md) and is load-bearing as of **6d**: [release.py](scripts/release.py) reads the type to size each package's version bump, mapping anything it does not recognise to a patch — so an invented type does not fail anywhere, it just silently ships as one. `rm(core):` and `steering:` are both in recent history.

  **Decided 2026-08-19: a hook, not a CI lint.** A PR-time check reports the convention was broken after the commits exist, which is the wrong end of the loop — it can only produce rewrite work. The enforcement that fits is a `commit-msg` hook refusing the commit as it is written. Prefer an off-the-shelf one (commitlint, conventional-pre-commit) over a bespoke script; note APM cannot deploy git hooks, so it installs out-of-band or via a `scripts/` bootstrap. Until then the convention is trusted, not enforced.

- [ ] **4h — Decide how upstream content gets read before a pin moves.** A pinned dependency is content an agent reads as instructions, and some of it ships scripts and hooks that run locally (`apm approve` gates *execution*, not content). The pin fixes **which** content arrives, not what it does, and a pin bump is the only moment where looking is cheap — the change is bounded and someone is already reviewing it.

...
- [x] **8c — Propagate the directive description standard** to the non-`meta-*` steering files. Rewrote every non-`meta-*` `description` to the directive + explicit-negative-constraint shape (front-loaded, third-person, sibling negative space where skills overlap): all 11 non-meta skills (`helm-*`, `k8s-standards`, `tf-standards`, `prd-*`, `troubleshooting`, `batch-task-execution`, `complex-research`, `mermaid-creator`), both prompts (`reflect`, `setup-mcp`), and the 9 remaining agents (`meta-updater` was already done in 8a). All pass the frontmatter hook. The `meta-updater` Phase 3 audit continues to flag any future divergences.

...
- [ ] **6a — Resolve unlicensed verbatim reuse from `arisng/github-copilot-fc` (blocks 6f).** Measured 2026-08-01 while annotating `took`: two local files reuse substantial verbatim text from a repo with **no LICENSE file**, i.e. no grant of rights at all — not permissive, not "probably fine". Making the marketplace repo public (**6f**) publishes it.

  | Local | Overlap with upstream |
  | --- | --- |
  | [mermaid-creator `references/unicode-symbols.md`](packages/core/.apm/skills/mermaid-creator/references/unicode-symbols.md) | **~99% verbatim** |
  | [mermaid-creator `references/mermaid-diagram-guide.md`](packages/core/.apm/skills/mermaid-creator/references/mermaid-diagram-guide.md) | **~88% verbatim** |
  | [mermaid-creator `SKILL.md`](packages/core/.apm/skills/mermaid-creator/SKILL.md) | ~49% (genuine condensation) |
  | [researcher-advanced.agent.md](packages/core/.apm/agents/researcher-advanced.agent.md) | ~41% verbatim |
...
- [ ] **6b — Reformulate the two unlicensed items until `took` can honestly read "Inspiration only."** The preferred resolution to **6a**, and the one that needs no cooperation from the upstream author. Rewrite each so the local content is genuinely original, then narrow its provenance from a bare `adaptedFrom` URL (which asserts whole-file derivation) to an `Inspiration only.` `took` entry. The `took` value is the acceptance test: if it cannot honestly say that, the rewrite is not done.

  | File | Work |
  | --- | --- |
  | [mermaid-creator `references/unicode-symbols.md`](packages/core/.apm/skills/mermaid-creator/references/unicode-symbols.md) | Near-total copy. Rebuild the symbol tables from scratch against the repo's own diagram conventions — the content is a symbol-to-meaning mapping, so an independent version is straightforward, just not a reshuffle of theirs |
  | [mermaid-creator `references/mermaid-diagram-guide.md`](packages/core/.apm/skills/mermaid-creator/references/mermaid-diagram-guide.md) | Mostly copied. Regenerate from the [official Mermaid docs](https://mermaid.js.org/), which are the real underlying source and permissively licensed — cite those instead |
  | [mermaid-creator `SKILL.md`](packages/core/.apm/skills/mermaid-creator/SKILL.md) | Already a genuine condensation; re-check the Unicode section and the three worked examples, which are lifted |
  | [researcher-advanced.agent.md](packages/core/.apm/agents/researcher-advanced.agent.md) | Rewrite the Research Approach, Quality Standards and Boundaries sections in the repo's own voice. The Output Authority contract and subagent-parallel execution are already original and stay |

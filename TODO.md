...
- [ ] **5e — Extend evals to third-party / integrated content.** Cover the pinned APM dependencies in `packages/*/apm.yml` and `adaptedFrom`/`mirror` content so an upstream update can be scored before adoption, and so vendored adaptations can be compared against their upstream original. Gated on 5a–5b proving out.
...
- [x] **6a — Resolve unlicensed verbatim reuse from `arisng/github-copilot-fc` (blocks 6f).** Measured 2026-08-01 while annotating `took`: two local files reuse substantial verbatim text from a repo with **no LICENSE file**, i.e. no grant of rights at all — not permissive, not "probably fine". Making the marketplace repo public (**6f**) publishes it.

  | Local | Overlap with upstream |
  | --- | --- |
  | ~~mermaid-creator (all three files)~~ | **Resolved 2026-08-02** — deleted and replaced by [authoring-mermaid-diagrams](packages/core/.apm/skills/authoring-mermaid-diagrams/SKILL.md), written against the official Mermaid docs. No `adaptedFrom` remains |
  | ~~researcher-advanced.agent.md~~ | **Resolved 2026-08-02** — body rewritten from scratch (see **6b**); provenance narrowed to an `Inspiration only.` `took` entry |

  Note this is the *opposite* of how these two were assumed to relate to upstream. Options: ask the author to add a licence; rewrite (see **6b**); or drop them. Decide before **6f** flips the marketplace public, and before **6c** generates `THIRD-PARTY-NOTICES.md` — a generated notice file cannot describe a licence that does not exist.

  Both entries are now resolved, so **6f** is no longer blocked on this item.

- [x] **6b — Reformulate the two unlicensed items until `took` can honestly read "Inspiration only."** The preferred resolution to **6a**, and the one that needs no cooperation from the upstream author. Rewrite each so the local content is genuinely original, then narrow its provenance from a bare `adaptedFrom` URL (which asserts whole-file derivation) to an `Inspiration only.` `took` entry. The `took` value is the acceptance test: if it cannot honestly say that, the rewrite is not done.

  | File | Work |
  | --- | --- |
  | ~~mermaid-creator~~ | **Done 2026-08-02.** Went further than reformulation: the skill was deleted and rebuilt as [authoring-mermaid-diagrams](packages/core/.apm/skills/authoring-mermaid-diagrams/SKILL.md) — new workflow, type selection keyed to the question the diagram answers, keywords and stability tiers verified against the Mermaid diagram detectors on `develop`, plus accessibility, theming and a validator script. The Unicode symbol reference was dropped rather than rewritten. Provenance is now `authoritativeSpec` only, so there is no `took` entry left to write |
  | ~~researcher-advanced.agent.md~~ | **Done 2026-08-02.** Body replaced wholesale rather than section-by-section: strand-based effort budget with tool-call ceilings, source tiers with version discipline, tool policy by capability category instead of a rotting server list, broad-to-narrow search with saturation stop rules, inline confidence labels with an explicit contradiction contract, and a fixed report skeleton persisted to `/memories/session/research-<slug>.md`. The Output Authority contract, the authority-notice header and subagent fan-out were kept as the locally-added fixes they are; fan-out gained a fallback for when the spawn tool is withheld at the nesting depth limit. Provenance is now an `Inspiration only.` `took` entry, re-pointed at upstream's post-restructure path (the old URL 404'd — see **4c**) |

  Do not treat paraphrasing as sufficient — the aim is content that stands on its own, not the same text with synonyms. When each file is done, replace its provenance with an `Inspiration only.` `took` entry and drop it from **6a**.

- [ ] **6c — Licensing.** Decide the licence for `.llmctl` itself (no `LICENSE` file exists, so the SBOM records NOASSERTION and the marketplace README currently has to say "unlicensed by default"), add it to both repos, then **generate** `THIRD-PARTY-NOTICES.md` from each bundle's embedded `apm.lock.yaml` instead of maintaining it by hand — the lockfile already records `repo_url` and per-file SHA-256 for every vendored skill. `apm pack` copies skill files only, so upstream `LICENSE` text has to be added deliberately: MIT needs the notice to travel, Apache-2.0 needs the licence, attribution notices, and a statement of changes.

  **Direction (unconfirmed, 2026-08-02).** Ideas to evaluate when this item is picked up, not decisions:

  - **Own files: a Creative Commons licence that requires attribution and permits everything else** — i.e. CC BY 4.0 shape (share, adapt, commercial, no share-alike), *as far as upstream licences allow*. Caveats to check: CC discourages CC licences for software, and this repo is mostly prose (skills, agents, instructions) plus a few scripts, so a split — CC BY for content, a permissive code licence for `scripts/` — may be cleaner than one licence for the whole tree.
  - **Adaptations need a compliant redistribution licence, not just attribution.** Anything beyond an `Inspiration only.` `took` entry carries upstream's terms with it: MIT/BSD portions keep their notice, Apache-2.0 portions keep the licence + NOTICE + a statement of changes, and none of them can be relicensed under CC BY. So the per-file provenance decides the per-file licence — the repo licence only covers what is genuinely original. Check that the `took` fidelity labels (`Inspiration only.` → `Largely derived.`) map cleanly onto "no obligation" → "full obligation".
  - **Automate licence propagation into the marketplace repo.** [scripts/pack-marketplace.py](scripts/pack-marketplace.py) should copy the repo `LICENSE` (and any per-package licence) into each bundle rather than relying on someone remembering — `apm pack` won't do it.
  - **Build `THIRD-PARTY-NOTICES.md` from metadata, not by hand.** Two sources: `metadata.provenance` on the steering files (`mirror` / `adaptedFrom` / `authoritativeSpec`) and `dependencies` in `packages/*/apm.yml` + the packed `apm.lock.yaml`. Neither currently records a licence, so this likely needs a **`license` property added to the `adaptedFrom` object form** alongside `url`/`took` (spec'd in [FRONTMATTER.md:93-103](packages/meta/.apm/skills/meta-skill/references/FRONTMATTER.md#L93-L103)), plus the equivalent for APM dependencies. If the field is added, [meta-updater](.apm/agents/meta-updater.agent.md) / [meta-upstream-sync](.apm/skills/meta-upstream-sync/SKILL.md) have to keep it fresh — an upstream relicence is exactly the kind of drift the audit should catch. Expect the generated notice to be *partial*: it can enumerate sources and licence IDs, but full licence texts still have to come from somewhere.
  - **Promote the `took` fidelity label from prose to its own field.** Today the label is the first sentence of a free-text string (`took: "Partly derived. The column-mapping rules."`), which is fine for a human but means anything generating notices has to parse prose to learn the obligation level. If the label drives which upstream terms apply, it should be structured — e.g. `fidelity: partly-derived` (enum) alongside a `took:` that carries only *what* was taken, next to the `license:` field above. Cheap to do: 11 provenance entries use the object form today, plus the spec in [FRONTMATTER.md:93-103](packages/meta/.apm/skills/meta-skill/references/FRONTMATTER.md#L93-L103), the mirror of it in [meta-agent/FRONTMATTER.md](packages/meta/.apm/skills/meta-agent/references/FRONTMATTER.md), the [CONTRIBUTING.md:39](CONTRIBUTING.md#L39) example, and [check-updates.ps1](.apm/skills/meta-upstream-sync/scripts/check-updates.ps1) — whose own example string is already stale against the spec (`"what was taken; what was not"`, while the spec says never record what was *not* taken). Decide alongside the `license` field so the object form changes once, not twice.
- [ ] **6d — Versioning.** `marketplace.versioning.strategy` is `lockstep`, so every package moves together and the packing script keeps `source:`/`version:` in sync. Decide what actually drives a bump (conventional commits? a release command? `apm publish`?), then automate it — including the upstream-dependency loop (`apm outdated` → `apm update` → re-pack → bump) so a pinned SHA moving is a normal release rather than a manual edit. Evaluate `per_package` versioning if lockstep becomes noisy.
- [ ] **6e — Release automation.** A workflow that packs, validates (`apm pack --check-versions --check-clean`, `claude plugin validate`), commits, and pushes the marketplace repo — either from `.llmctl` CI (needs a cross-repo token) or a scheduled job in the marketplace repo. Ties into **4**: the same gates belong on PRs here.
- [ ] **6f — Go public and verify end to end.** The marketplace repo is private, which is fine for `claude plugin marketplace add <local path>` but blocks Cowork and any other host that clones anonymously. Flip it public when the licensing in **6c** is settled, then install a plugin end to end from Cowork and record what actually loaded (skills, commands, and whether 0.26's packed `agents/`, `instructions/`, and `.mcp.json` are honoured — the previous finding that they do not travel predates 0.26).
- [ ] **6g — Manifest schema mismatch.** `claude plugin validate` warns that the marketplace description belongs under `metadata.description` while APM emits it top-level. Confirm whether Claude reads the top-level field anyway; if not, raise it upstream with microsoft/apm rather than post-processing the generated file.

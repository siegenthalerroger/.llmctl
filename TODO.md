...
- [ ] **4l — `apm install --frozen` cannot restore a package that declares MCP servers.** Found 2026-09-18, when `packages/core` became a root dependency and CI's deploy step went red on a clean checkout: `apm install --frozen` aborts with "package manifest not found at `apm_modules/blader/humanizer/apm.yml`; re-run `apm install` to restore it" before it fetches anything. Isolated on APM 0.31.0 to the `dependencies.mcp` block alone — the same three dependencies with `mcp: []` restore frozen without complaint, and a warm `apm_modules/` hides it entirely. It affects `core` and `ops`, and it is the reason the workflows' deploy step installs unfrozen. Related to the R1 note behind [Lockfiles](CONTRIBUTING.md#lockfiles): `--frozen` was already the wrong tool for holding a lockfile to its pins, which is the `lockfiles` gate's job. Report it upstream, and drop the workaround when a release fixes it.

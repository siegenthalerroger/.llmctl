...
- **Tooling:** a uv project — [pyproject.toml](pyproject.toml), one module per command under `src/llmctl/`, one `llmctl-<command>` console script each — run with `uv run`, which syncs the locked environment first. `uv.lock` moves only through `uv lock`. Lint and types are **ruff** and **ty**, configured in `pyproject.toml` and pinned in the `dev` dependency group: run `uv run ruff format src/ && uv run ruff check src/ && uv run ty check src/` before committing, because the `tooling` gate runs exactly those three plus `uv lock --check`. The `scripts:` block in [apm.yml](apm.yml) is the index: `check`, `update`, `check-updates`, `check-steering`, `versions`, `release`, `pack-marketplace`, in the order you would run them.

...
class Runner:
    ...
    def run(self, ctx: Context) -> int:
        ...
        ctx.console.print("check: %s" % ctx.repo)
        for gate in self.gates:
            if self.only and gate.key not in self.only:
                continue
            ctx.console.print("- [bold]%-12s[/] %s" % (gate.key, gate.title))
    ...
    def _outcome(self, ctx: Context, gate: Gate) -> Outcome:
        ...
        for need in gate.needs:
            ...
            if available:
                continue
            reason = "needs %s: %s" % (need.name, why)
        ...
        try:
            ...
            if os.environ.get("LLMCTL_DEBUG"):
                traceback.print_exc()
            return fail(("%s: %s" % (type(exc).__name__, exc))[:200])
    ...
    def _record(self, ctx: Context, gate: Gate, outcome: Outcome) -> None:
        ...
        ctx.console.print("  [%s]%s[/]%s" % (STYLE[outcome.status],

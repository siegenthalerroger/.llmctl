...
2. **Map each server** to an `apm.yml` entry per the [meta-mcp reference](../skills/meta-mcp/references/mcp-configuration.md): `name`, `registry: false`, `transport` (infer `stdio` from `command`, `http`/`sse` from `url`), plus `command`/`args`/`env` or `url`/`headers`.

...
- **Plugins are an export, not a parallel structure.** Each sub-package carries a thin `packages/<name>/.claude-plugin/plugin.json` pointing at its `.apm/` dirs, and the root `apm.yml` holds a `marketplace:` block. `apm pack` generates the marketplace manifests — `.claude-plugin/marketplace.json` (Claude) and `.agents/plugins/marketplace.json` (Codex) — from that block (see [meta-plugin](packages/meta/.apm/skills/meta-plugin/SKILL.md)). This marketplace bridges the one surface APM can't reach: **claude.ai's Cowork** (add via `/plugin marketplace add siegenthalerroger/.llmctl`; also Claude Desktop/Code). It does NOT reach claude.ai Chat or hosted ChatGPT.
- **The plugin path is reduced-fidelity; `apm install` remains the full deploy.** A plugin/marketplace install carries only **skills** and **commands** (prompts). **Instructions, MCP servers, and agents do NOT travel** through a Claude/Cowork plugin (no instructions component; `apm pack` drops MCP; the plugin `agents` field does not load). Keep this in mind when relying on the marketplace — use `apm install` where those primitives matter.
- **The two generated `marketplace.json` files are committed** as the single explicit exception to "don't commit machine-generated wiring" (they are the published catalog and must be in-repo for Cowork to clone). Regenerate with `apm pack` after editing the `marketplace:` block; `build/` (scratch bundles) is git-ignored. Keep every package + the marketplace at the same version (`lockstep`).
